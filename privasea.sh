...
echo "Selamat datang! Skrip ini akan mengatur Docker untuk Anda."
echo "Anda dapat memilih antara menggunakan Docker dengan mode rootless atau memberikan akses non-root ke daemon Docker."
echo "Pilih salah satu opsi berikut:"
echo "1. Gunakan Docker dalam rootless mode (keamanan tinggi)."
echo "2. Berikan akses pengguna non-root ke Docker daemon."
...
if [[ $choice -eq 1 ]]; then
    ...
    echo "Memberikan akses non-root ke Docker daemon..."

    if getent group docker > /dev/null 2>&1; then
        echo "Grup 'docker' sudah ada, melewati pembuatan grup."
    else
    ...
    echo "Periksa apakah Docker dapat berjalan tanpa sudo:"
    docker run hello-world
    if [[ $? -eq 0 ]]; then
        echo "Docker berhasil dijalankan tanpa sudo!"
    else
        echo "Ada masalah, pastikan Anda sudah keluar dan masuk kembali ke terminal."
    fi

else
    echo "Pilihan tidak valid. Harap jalankan ulang skrip dan pilih antara 1 atau 2."
    exit 1
fi
